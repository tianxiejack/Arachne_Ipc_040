...
SerialSet Serial[1] = { "/dev/ttyUSB0", 115200 };
...
void SetReturnPack(Return_pack* value) {
	...
	return_pack = *value;
	return_pack.payload.Track_Deviation_X*=10;
	return_pack.payload.Track_Deviation_Y*=10;
...
void InitIPCModule() {
	...
	/*创建线程*/
	IPC_pthread_create(PTHREAD_NAME, &thread[0], NULL, Recv_ipc, NULL);
	IPC_pthread_create(PTHREAD_NAME1, &thread[1], NULL, SendMessage, NULL);
}
